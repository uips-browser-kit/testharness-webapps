"""
Generate YAML test fixtures from harness.yaml and src/core/.

Outputs:
  tests/fixtures/apps/{app.id}/profile.yaml   — App profile
  ...
  infra/hosts.txt                             — /etc/hosts entries for all app environments
...
from src.core.models import App, NotServerVisible, Route  # noqa: E402
from src.core.resolver import resolve  # noqa: E402
...
def resolve_cases(app: App) -> list[dict]:
    ...
    for route in app.routes:
        for env_id in app.environments:
            try:
                url = resolve(app, route.id, env_id, SAMPLE_PARAMS)
                params = {
                    k: SAMPLE_PARAMS[k] for k in _relevant_params(route) if k in SAMPLE_PARAMS
                }
                cases.append(
                    {
                        "route_id": route.id,
...
def main() -> None:
    ...
    write_hosts_txt(Path(__file__).parent.parent / "infra" / "hosts.txt", apps)
